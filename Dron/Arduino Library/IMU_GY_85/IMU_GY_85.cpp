...
void IMU_GY_85::GyroCalibrate()
{
	...
	for (uint8_t i = 0; i < 100; i++)  //take the measurement from 10 gyro probes and divide it from the current probe
	{
		delay(10);
		double* gp = readGyro();
		tmpx += *(gp);
...
double* IMU_GY_85::readGyro()  //podatek vrne v deg/s
{
	...
	// yos: +30.7625
	// z-os: -2.0125
	axis[0] = (((buff[4] << 8) | buff[5]) ) / 14.375 - g_offx/2.0;		// x-os
	axis[1] = (((buff[2] << 8) | buff[3]) ) / 14.375 - g_offy/2.0;		// y-os
	axis[2] = (((buff[6] << 8) | buff[7]) ) / 14.375; -g_offz/2.0;		// z-os
